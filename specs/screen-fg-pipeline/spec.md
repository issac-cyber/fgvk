...
- **`capture_mode`（window \| monitor）**：本機 GNOME portal 的視窗 picker **不會顯示**（`SelectSources` 立即回 `success`+空 `parent`、無遮罩），導致 spec 的 window 捕捉流程卡死在「沒有選到視窗」。新增 config key `capture_mode`（預設 `"window"` = spec 原行為：`SelectSources` 帶 `types=2`（WINDOW）；`"monitor"` = 全螢幕 monitor 捕捉：`SelectSources` 帶 `types=1`（MONITOR）、無 picker、tolerate 空 `parent`）。`monitor` 為 spec 外的逃生門，不改變 `window` 預設。
  - **GLib 2.88 陷阱**：`GVariantBuilder` 的 `{sv}` 對 GVariant 參數是「借用」（存指標、不 inc ref、`end()` 也不 copy）→ unref 該參數會掛空指標、序列化無限遞迴 segfault（實測）。makeSelectDict / makeDict / makeCreateSessionDict 因此對所有新造的 GVariant 參數**故意不 unref**（dict 引用它們、活到 dict 釋放）。
